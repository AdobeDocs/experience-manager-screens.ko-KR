...
1. **환경 설정** 패널을 선택합니다. 이 두 옵션 중에서 플레이어에 대해 **관리 UI** 및 **채널 전환기**&#x200B;를 활성화/비활성화할 수 있습니다.
...
* AEM Screens 플레이어에 유지할 최대 로그 파일 수를 **최대 아니요. 을 사용하여**&#x200B;을(를) 유지합니다.

* Screens 플레이어에 대해 **관리 메뉴**, **채널 전환기** 및 **활동 UI**&#x200B;를 활성화 또는 비활성화합니다.

   **활동 UI**&#x200B;가 **구성** 메뉴에서 활성화된 경우 AEM Screens 플레이어는 아래 그림과 같이 플레이어의 오른쪽 위 모서리에 *플레이어 활동 알림*&#x200B;을 표시합니다.
...
| AEM Screens Player 충돌 | AEM Screens Player가 항상 실행되도록 하는 감시 응용 프로그램을 사용하는 것이 좋습니다 |
| AEM Screens Player 설정 분실 | AEM 서버에 대한 연결 확인 |
| AEM Screens Player는 Player를 다시 시작하거나 다시 부팅한 후 자동으로 시작되지 않습니다. | OS 시작 폴더 또는 초기화 프로시저 확인 |
| AEM Screens Player에 잘못된/이전 컨텐츠가 표시됩니다. | 네트워크 연결 확인 |

### AEM Screens 플레이어 업데이트 {#updates-for-aem-screens-player}

AEM Screens 플레이어용으로는 두 가지 유형의 업데이트가 있습니다.

| **메서드** | **세부 사항** | **Via Remote** | **자동화** | **0 다운타임** |
|---|---|---|---|---|
| 펌웨어 업데이트 | 원격 명령을 통해 설치된 기존 플레이어에 적용됩니다. 업데이트 후에는 기존 컨텐츠가 플레이어에 자동으로 다시 로드됩니다. | 예 | 사용자 지정 | 거의 - 1-3초 |
| 플레이어 쉘 업데이트 | 플레이어에 배포할 새로운 실행 파일입니다. 이 방법을 사용하려면 플레이어에서 새 바이너리를 원격 복사하고 현재 실행 중인 버전을 중지하고 새 버전을 시작해야 합니다. 이 경우 사전 로드된 패키지를 다시 다운로드해야 할 수 있습니다. | 예(원격 셸을 통해) | 사용자 지정 | 아니오 |
