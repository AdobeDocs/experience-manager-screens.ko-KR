...
1. 선택 **채널 할당** 작업 표시줄에서 을 참조하십시오.

   또는,

   선택 **대시보드** 및 선택 **+채널 할당** 다음에서 **할당된 채널** 패널 을 클릭하여 **채널 할당** 대화 상자.
...
* **이름순** - 컨텍스트별로 실제 채널로 확인되는 채널의 이름을 입력합니다. 이 기능을 사용하면 위치별 콘텐츠를 동적으로 해결하는 채널의 로컬 버전을 만들 수 있습니다. (예: 이름이 인 채널) *오늘의 거래*, 여기서 두 도시의 실제 콘텐츠는 다르지만 모든 디스플레이에는 여전히 올바른 채널 역할이 있습니다.
...
* **현재 항목 끝** - 새 일정이 활성화되거나 업데이트가 수신되면 선택적으로 시퀀스의 현재 항목이 재생을 완료할 때까지 대기할 수 있습니다. 그 후에만 새 콘텐츠를 새로 고치거나 재생할 수 있습니다.
...
>
>* [자산에서 반복 처리](https://experienceleague.adobe.com/en/docs/experience-manager-screens/user-guide/authoring/product-features/asset-level-scheduling)
>* [채널에서 에셋에 대한 반복 처리](https://experienceleague.adobe.com/en/docs/experience-manager-screens/user-guide/authoring/product-features/channel-level-activation)
