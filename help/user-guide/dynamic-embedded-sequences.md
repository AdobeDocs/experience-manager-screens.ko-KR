...
동적 포함 시퀀스를 사용하려면 다음 방법으로 채널을 지정합니다 ***채널 역할***. 채널 역할은 디스플레이의 컨텍스트를 정의합니다. 역할은 다양한 작업을 대상으로 하며 역할을 수행하는 실제 채널과는 독립적입니다. 이 섹션에서는 역할별로 채널을 정의하는 사용 사례와 해당 콘텐츠를 글로벌 채널에 적용하는 방법에 대해 설명합니다. 또한 역할을 할당에 대한 식별자나 컨텍스트에서 채널에 대한 별칭으로 생각할 수 있습니다.
...
참조 *역할별 채널*&#x200B;를 사용하면 채널의 로컬 버전을 만들 수 있습니다. 이렇게 하면 위치별 콘텐츠를 동적으로 해결하고 위치별 채널에 콘텐츠를 사용하는 전역 채널을 만들 수도 있습니다.
...
1. **각 위치 폴더 아래에 두 개의 위치와 채널을 만듭니다.**
   ...
   1. 클릭 **지역 A** 및 클릭 **+ 만들기** 작업 표시줄에서
   1. 클릭 **위치** 을 사용하여 마법사에서 **제목** 다음으로: **스토어 1**. 마찬가지로 마법사에서 이라는 다른 위치를 만듭니다. **스토어 2** (으)로 **제목** 다음으로: **스토어 2**. 다음을 떠나실 수 있습니다. **이름** 만드는 동안 필드가 비어 있음 **스토어 1** 및 **스토어 2**.
   1. 단계 (b)를 반복하고 지금 클릭 **시퀀스 채널** 을 클릭합니다. 다음을 입력합니다. **제목** 다음으로: **지역 A** 및 **이름** (으)로 **지역** 이 채널용입니다.

   >[!CAUTION]
   >
   >채널을 만드는 동안 다음을 확인하십시오 **지역 A**&#x200B;을(를) 입력한 후 **제목** 다음으로: **지역 A** 및 **이름** as a **지역**.
...
1. **각 위치 아래에 디스플레이 및 채널을 만듭니다.**
   ...
   1. 클릭 **스토어 1** 및 클릭 **+ 만들기** 작업 표시줄에서
   1. 클릭 **표시** 마법사에서 만들고 **`Store1Display`**.
   1. 단계 (b)를 반복하고 이번에는 클릭 **시퀀스 채널** 을 클릭합니다. 다음을 입력합니다. **제목** 다음으로: **`Store1Channel`** 및 **이름** 다음으로: **스토어**.
...
1. **일정 만들기**
   ...
   다음 위치로 이동하여 **일정** AEM Screens 폴더를 추가합니다. 그런 다음 **만들기** 작업 표시줄에서

   다음 이미지는 **AdSchedule** 에서 생성됨 **데모** 프로젝트.
...
1. **예약에 채널 할당**
   ...
   1. 클릭 **+ 채널 할당** 다음에서 **할당된 채널** 패널 을 클릭하여 **채널 할당** 대화 상자.
   1. 클릭 **참조 채널** 경로 기준.
   1. 다음을 클릭합니다. **채널 경로**, 다음과 같음 **데모** > ***채널*** > ***글로벌***.
   1. 다음을 입력합니다. **채널 역할**, 다음과 같음 **전역 세그먼트**.
   1. 다음을 클릭합니다. **지원되는 이벤트**, 다음과 같음 **초기 로드**, **유휴 화면**, 및 **사용자 상호 작용**.
   1. **저장**&#x200B;을 클릭합니다.

   **지역에 대한 역할별 채널 할당:**

   1. 클릭 **+ 채널 할당** 다음에서 **할당된 채널** 패널.
   1. 채널 할당 대화 상자에서 **참조 채널** 이름으로.
...
1. **전역 채널에 동적 포함 시퀀스를 구성합니다.**
   ...
   마찬가지로 다른 구성 요소를 클릭하고 속성을 열어 **채널 할당 역할** 다음으로: **StoreAdsegment**.
...
1. **각 디스플레이에 일정 할당**
   ...
   1. 예약에 대한 경로를 클릭합니다(예: 여기 **데모** > **일정** > **AdSchedule**).
   1. **저장**&#x200B;을 클릭합니다.
...
1. **사용자 만들기 및 그룹에 사용자 추가**
   ...
   1. 다음을 클릭합니다. **그룹** 을(를) 탭하고 의 그룹 이름을 입력합니다. **그룹 클릭**&#x200B;예를 들어 다음을 입력합니다. **Global-Author** 추가하려면 **전역 사용자** 특정 그룹에 연결합니다.
   1. **저장 및 닫기**&#x200B;를 클릭합니다.
...
1. **기여자에게 모든 그룹 추가**
   ...
   1. 다음으로 이동 **Adobe Experience Manager**.
   1. 클릭 **도구** > **보안** > **그룹**.
   1. 클릭 **참가자** 목록에서 을(를) 클릭하고 **구성원** 탭.
   1. 다음을 클릭합니다. **그룹** 과 같은 **Global-Author**, **Region-Author,** 및 **Store-Author** 기여자를 위해.
   1. **저장 및 닫기**&#x200B;를 클릭합니다.

1. **각 그룹에 대한 권한 액세스**

   1. 다음 위치로 이동 *사용자 관리자* 및 이 UI를 사용하여 다른 그룹에 대한 권한을 수정할 수 있습니다.
   1. 검색 대상 **Global-Author** 을(를) 클릭하고 **권한** 아래 그림과 같이 탭으로 이동합니다.
...
1. **각 그룹에 대한 권한 수정**
   ...
   **Global-Author용:**
   ...
   다음은 를 보여 줍니다. **전역 사용자** 에 대한 액세스 권한이 있습니다. **글로벌 채널**. 및, 둘 다에 액세스 **지역 A** 및 **지역** 4개의 모든 상점을 가진 **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4**.
   ...
   **Region-Author용:**
   ...
   다음은 지역 사용자가 두 항목에 모두 액세스할 수 있음을 보여 줍니다 **지역 A** 및 **지역**. 그리고, 4개의 모든 상점들에 대한 접근, 즉 **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4**, 그러나 은 다음에 액세스하지 않습니다. **글로벌** 채널.
   ...
   **Store-Author용:**
   ...
   다음은 를 보여 줍니다. **Store-User** 에 대한 액세스 권한만 있음 **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4**. 그러나 액세스 권한은 없습니다. **글로벌** 또는 영역(**지역 A** 및 **지역**) 채널.
