...
description: 이 페이지를 따라 AEM Screens 프로젝트에 동적 포함된 시퀀스를 구현하는 방법을 알아봅니다.
...
채널을 디스플레이에 할당하는 동안 디스플레이 경로 또는 컨텍스트별로 실제 채널로 확인될 채널의 역할을 지정할 수 있는 옵션이 있습니다.

동적 포함 시퀀스를 사용하려면 다음 방법으로 채널을 지정합니다 ***채널 역할***. 채널 역할은 디스플레이의 컨텍스트를 정의합니다. 역할은 다양한 작업이 타겟팅하며 역할을 수행하는 실제 채널과는 독립적입니다. 이 섹션에서는 역할별로 채널을 정의하는 사용 사례와 해당 콘텐츠를 글로벌 채널에 활용하는 방법을 설명합니다. 또한 역할을 할당에 대한 식별자나 컨텍스트에서 채널에 대한 별칭으로 생각할 수 있습니다.

### 동적 임베디드 시퀀스 사용의 이점 {#benefits-of-using-dynamic-embedded-sequences}

채널 폴더 대신 위치 내에 시퀀스 채널을 배치하면 로컬 또는 지역 작성자가 계층 구조 상단에 있는 채널 편집으로 제한되면서 관련 콘텐츠를 편집할 수 있다는 이점이 있습니다.

참조 *역할별 채널*&#x200B;를 사용하면 채널의 로컬 버전을 만들어 위치별 콘텐츠를 동적으로 해결할 수 있고, 위치별 채널에 대한 콘텐츠를 활용하는 글로벌 채널을 만들 수도 있습니다.
...
>
>포함된 시퀀스 구현에 대한 자세한 내용은 을 참조하십시오. [포함된 시퀀스](embedded-sequences.md) AEM Screens.

다음 예제에서는 다음 주요 용어에 초점을 맞춰 솔루션을 제공합니다.

* a ***주계열채널*** 글로벌 시퀀스의 경우
* ***동적 포함 시퀀스*** 시퀀스의 각 로컬 사용자 지정 가능 부분에 대한 구성 요소
...
이 기능의 구현을 시작하기 전에 동적 포함 시퀀스 구현을 시작할 수 있는 다음 사전 요구 사항이 준비되었는지 확인하십시오.

* AEM Screens 프로젝트 만들기(이 예에서는 **데모**)

* 다음으로 채널 만들기 **글로벌** 아래에 **채널** 폴더

* 에 콘텐츠 추가 **글로벌** 채널 (*다음을 확인하십시오.**Resources.zip**관련 자산용*)
...
>[!CAUTION]
>
>동적 임베디드 시퀀스를 구현하는 동안 **이름** 및 **제목** 각 위치에서 채널을 만드는 동안 필드가 표시됩니다. 명명법에 대한 지침을 주의 깊게 따르세요.
...
1. **각 위치 폴더 아래에 두 개의 위치와 채널을 만듭니다.**
   ...
   >[!CAUTION]
   >
   >에서 생성된 채널에 대해 동일한 이름을 사용할 수 있는지 확인하십시오. **지역 A** 및 **지역** 다음으로: **지역**.
...
1. **각 위치 아래에 디스플레이 및 채널을 만듭니다.**
   ...
   >이 예에서 채널은 **지역 A** 및 **지역** 공유하기 **이름** 다음으로: **지역** 및 채널 **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4** 공유하기 **이름** 다음으로: **스토어**.

   ![screen_shot_2018-09-19at120206pm](assets/screen_shot_2018-09-19at120206pm.png)

   마찬가지로 디스플레이를 만드는 방법: **Store2Display** 및 채널 **Store2Channel** 아래에 **스토어 2** (다음으로 이름 포함) **스토어**).

   >[!NOTE]
   >에서 생성된 채널에 대해 동일한 이름을 사용할 수 있는지 확인하십시오. **스토어 1** 및 **스토어 2** 다음으로: **스토어**.

   ![screen_shot_2018-09-19at120329pm](assets/screen_shot_2018-09-19at120329pm.png)

   이전 단계에 따라 채널을 만들고 **스토어 3** 및 **스토어 4** 아래에 **지역**. 다시 한 번, 동일한 코드를 사용해야 합니다. **이름** 다음으로: **스토어** 채널을 만드는 중 **Store3Channel** 및 **Store4Channel** 각각.

   다음 이미지는에서 디스플레이 및 채널을 보여 줍니다 **스토어 3**.

   ![screen_shot_2018-09-19at120448pm](assets/screen_shot_2018-09-19at120448pm.png)

   다음 이미지는에서 디스플레이 및 채널을 보여 줍니다 **스토어 4**.
...
1. **일정 만들기**
   ...
   탐색 및 선택 **일정** 폴더를 AEM Screens 프로젝트에 추가하고 **만들기** 작업 표시줄에서 새 일정을 만듭니다.
...
1. **예약에 채널 할당**
   ...
   1. 클릭 **+ 채널 할당** 출처: **할당된 채널** 패널 을 열어 **채널 할당** 대화 상자.
   1. 선택 **참조 채널**.. 경로 기준.
...
1. **전역 채널에 동적 포함 시퀀스를 구성합니다.**
   ...
   다음 위치로 이동 **글로벌** 채널, 처음에 생성한 위치 **데모** 프로젝트.

   클릭 **편집** 를 입력하여 편집기를 엽니다.

   ![screen_shot_2018-09-13at52754pm](assets/screen_shot_2018-09-13at52754pm.png)

   두 개 드래그 앤 드롭 **동적 포함된 시퀀스** 채널 편집기의 구성 요소입니다.
...
표시 경로를 다음과 같이 지정하는 경우 **데모** —> **위치** —> **지역 A** —> **스토어 1** —> **Store1Display**, 다음 콘텐츠가 AEM Screens 플레이어에 표시됩니다.

![channeldisplay1](assets/channeldisplay1.gif)

**시나리오 1**:

표시 경로를 다음과 같이 지정하는 경우 **데모** —> **위치** —> **지역** —> **스토어 3** —> **Store3Display**, 다음 콘텐츠가 AEM Screens 플레이어에 표시됩니다.
...
위치에 따라 콘텐츠에 대한 사용자 액세스를 제한하려면 ACL을 수정해야 합니다.
...
>[!NOTE]
>
>각 개인 또는 팀이 자체 프로젝트를 처리하도록 ACL을 사용하여 프로젝트를 분리하는 방법에 대한 자세한 내용은 을 참조하십시오. **ACL 설정**.
...
1. **각 그룹에 대한 권한 수정**
   ...
   **Global-Author용:**
   ...
   1. 다음으로 이동 ***/content/screens/demo/locations/region-a*** 모든 권한을 확인합니다. 마찬가지로 다음에 대한 권한을 확인합니다. **지역**.

   다음 단계를 이해하려면 아래 그림을 참조하십시오.
   ![screen_shot_2018-09-18at115752am](assets/screen_shot_2018-09-18at115752am.png)

   다음 이미지는 이제 **전역 사용자** 에 대한 액세스 권한이 있습니다. **글로벌 채널** 및 둘 다 **지역 A** 및 **지역** 네 가게 모두 **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4**.
   ...
   **Region-Author용:**
   ...
   다음 이미지는 이제 지역 사용자가 **지역 A** 및 **지역** 네 가게 모두 **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4** 에 액세스하지 않음 **글로벌** 채널.
   ...
   **Store-Author용:**
   ...
   1. 다음으로 이동 ***/content/screens/demo/locations*** 다음 항목만 확인 **읽기** 사용 권한.
   1. 다음으로 이동 ***/content/screens/demo/channels*** 다음에 대한 권한 선택 취소 **글로벌** 채널.
   1. 다음으로 이동 ***/content/screens/demo/locations/region-a*** 다음 항목만 확인 **읽기** 사용 권한. 마찬가지로 **읽기** 다음에 대한 권한 **지역**.
   ...
   다음 이미지는 이제 **Store-User** 는 네 개의 상점에만 액세스할 수 있습니다. 즉, **스토어 1**, **스토어 2**, **스토어 3**, 및 **스토어 4** 에 액세스할 수 있는 권한이 없습니다. **글로벌** 또는 영역(**지역 A** 및 **지역**) 채널.

   ![스토어](assets/store.gif)

>[!NOTE]
>
>권한 설정에 대한 자세한 내용은 을 참조하십시오. [ACL 설정](setting-up-acls.md).
