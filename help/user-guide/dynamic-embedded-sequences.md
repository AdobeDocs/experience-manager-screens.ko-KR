...
>**포함된 시퀀스와 동적 포함된 시퀀스**
>
>동적 포함된 시퀀스는 포함된 시퀀스와 유사하지만 하나의 채널에 대한 변경 사항/업데이트가 관련된 다른 채널에 전파되는 계층 구조를 사용자가 따를 수 있습니다. 부모-자식 계층 구조를 따르며 이미지나 비디오와 같은 자산도 포함합니다.
>
...
1. **각 위치 아래에 디스플레이 및 채널 만들기를 참조하십시오.**
   ...
   >[!NOTE]
   >**store 1** 및 **스토어 2**&#x200B;에서 만든 채널에 대해 &lt;a4/>store **와 동일한 이름을 사용할 수 있는지 확인하십시오.**
...
1. **각 위치의 채널에 컨텐츠를 추가합니다.**
   ...
   **데모** -> **위치** -> **지역 A** -> **지역 A**&#x200B;으로 이동하고 작업 표시줄에서 **편집**&#x200B;을 클릭합니다. 채널에 추가할 자산을 드래그하여 놓습니다.
   ...
   마찬가지로, **데모** -> **위치** -> **지역 B** -> **지역 B**&#x200B;을 클릭하고 작업 표시줄에서 **편집**&#x200B;을 클릭하여 아래 표시된 대로 자산을 채널에 드래그하여 놓습니다.
...
1. **전역 채널에 동적 포함된 시퀀스 구성을 참조하십시오.**
   ...
   **전역** 채널로 이동합니다. **Demo** 프로젝트에서 새로 만들었습니다.
   ...
   채널 편집기에서 2개의 **동적 포함된 시퀀스** 구성 요소를 드래그하여 놓습니다.
...
표시 경로를 **Demo** —> **위치** —> **지역 A** —> **스토어 1** —> **Store1Display**&#x200B;로 할당하면 다음 컨텐츠가 AEM Screens 플레이어에 표시됩니다.
...
1. **모든 그룹을 작성자에게 추가**
   ...
   1. 작성자에게 **Global-Author**, **Region-Author,** 및 **Store-Author**&#x200B;과 같은 &lt;a0/>그룹&#x200B;**을 선택합니다.**
   1. **저장 후 닫기**&#x200B;를 클릭합니다.
...
1. **각 그룹에 대한 권한 수정**
   ...
   **지역 작성자의 경우:**
   ...
   다음 이미지는 이제 지역-사용자가 **스토어 1**, **스토어 2**, **스토어 3** 및 &lt;a1> 등 4개 스토어를 포함하는 **지역 A** 및 **지역 B**&#x200B;에 모두 액세스할 수 있음을 보여줍니다. 0/>4 **스토어 4**&#x200B;전역&#x200B;**채널에 액세스할 수 없습니다.**
