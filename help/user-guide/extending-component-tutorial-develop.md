...
A `Custom Poster` 구성 요소는 이미지 구성 요소를 확장하여 만듭니다.
...
1. 공유 클라이언트 라이브러리에 다음 속성을 추가합니다.
   ...
   * `categories` | 문자열[] | `cq.screens.components`
   ...
   다음 `categories` 속성은 클라이언트 라이브러리를 식별하는 문자열입니다. 다음 `cq.screens.components` 범주는 편집 모드와 미리 보기/프로덕션 모드 모두에서 사용됩니다. 따라서 CSS/JS가에 정의되어 있습니다. `shared` clientlib은 모든 모드에서 로드됩니다.

   에 직접 경로를 노출하지 않는 것이 좋습니다. `/apps` 프로덕션 환경에서. 다음 `allowProxy` 속성은 클라이언트 라이브러리 CSS 및 JS가 접두사를 통해 참조되도록 합니다. `/etc.clientlibs`. 에 대한 추가 정보 [allowProxy 속성은 여기에서 찾을 수 있습니다.](https://experienceleague.adobe.com/en/docs/experience-manager-65/content/implementing/developing/introduction/clientlibs)
...
아래 비디오는 완료된 구성 요소와 시퀀스 채널에 구성 요소를 추가하는 방법을 보여 줍니다. 그런 다음 채널은 위치 표시에 추가되고 최종적으로 스크린 플레이어에 할당됩니다.
