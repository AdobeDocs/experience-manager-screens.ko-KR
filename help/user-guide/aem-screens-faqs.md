...
>[!NOTE]
>문제가 발생하기 전에 기본 지원 또는 고객 지원 센터에서 테스트해야 하는 필수 확인 목록입니다.
...
* 예약된 컨텐츠가 있는지, 그리고 시간이 올바른지 확인하십시오. 플레이어에 설정된 시간이 올바른지 확인하십시오.
* 플레이어 콘솔 로그를 확인하고 오류가 있는지 확인하십시오. 콘솔 로그를 보려면 마우스 오른쪽 단추를 클릭하고 검사합니다. windows 플레이어를 사용하는 경우 `CTRL + ALT +I`을 눌러 개발 콘솔을 불러와서 로그를 확인합니다.
