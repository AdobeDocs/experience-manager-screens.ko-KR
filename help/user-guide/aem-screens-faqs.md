...
1. 작성자 `/etc/map` 또는 *게시* 인스턴스에 대한 규칙이 있는지 *확인하고 화면 경로가 다른 경로로* `sling:match` 내부적으로 리디렉션되었는지 확인합니다. 의 정확한 url `/system/console/jcrresolver` 을 확인하면 *게시* 인스턴스가 이러한 URL을 다른 경로로 재작성하고 있는지 확인하는 데 도움이 됩니다.
