...
* 시스템에서 실행 중인 플레이어를 사용하여 `http://localhost:24502` 로 이동합니다. 모든 컨텐츠가 올바르게 다운로드되었는지 확인합니다.
* 적절한 표현물이 만들어지고 올바른 표현물이 재생되고 있는지 자산을 확인합니다.
* 예약된 컨텐츠와 시간이 올바른지 확인합니다. 플레이어에서 설정된 시간이 올바른지 확인합니다.
* Inspect 플레이어 콘솔이 로그인하고 오류를 확인합니다. 마우스 오른쪽 단추를 클릭하고 검사하여 콘솔 로그를 확인합니다. Windows 플레이어를 사용하는 경우 `CTRL + ALT +I` 키를 눌러 개발 콘솔을 표시하여 로그를 확인합니다.
...
*동적 참조*&#x200B;의 경우 채널이 디스플레이에 할당되면(스케줄이 아님) 해상도가 발생합니다. 표시 경로가 채널의 컨텍스트가 되고 해상도는 다음과 같이 발생합니다(최고 - 최저 우선순위).
...
1. Android 플레이어 설정 —> **정보**&#x200B;로 이동합니다.
1. 빌드 번호를 7번 탭하여 **설정**&#x200B;에서 **개발자 옵션**&#x200B;을 사용하도록 설정
1. **개발자 옵션**&#x200B;으로 이동합니다.
1. **계속 깨어있음** 사용
...
1. AEM Screens 플레이어가 시작되면 `/content/screens/svc.json`에 요청됩니다. 플레이어가 응답에 404 상태 코드를 받으면 *publish* 인스턴스에 대해 `/libs/granite/core/content/login.validate/j_security_check`을 사용하여 인증 요청을 시작합니다. *publish* 인스턴스에 사용자 지정 오류 처리기가 있는 경우 `/content/screens/svc.json` 또는 `/content/screens/svc.ping.json`에서 익명 사용자에 대한 404 상태 코드를 반환해야 합니다.

1. Dispatcher 구성이 `/filters`에서 이러한 요청을 허용하는지 확인합니다.
...
1. *author* 또는 *publish* 인스턴스 및 화면 경로가 `sling:match`에 일치하고 내부적으로 다른 경로로 리디렉션되는지 확인합니다. `/etc/map` `/system/console/jcrresolver`의 정확한 URL을 확인하는 것은 *publish* 인스턴스가 이러한 URL을 다른 경로에 다시 쓰고 있는지 식별하는 데 도움이 됩니다.
...
1. ***Livefyre 폴러를 비활성화합니다.***
   ...
   * CRXDE Lite에서 `/etc/importers/polling/livefyre-poller/jcr:content` 로 이동합니다.
