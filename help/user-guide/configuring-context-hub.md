...
AEM Screens 프로젝트에서 인벤토리 기반 채널을 만들고 관리하는 세부 사항을 살펴보려면, 여러 시나리오와 관련이 있고 중요한 주요 용어 중 일부를 알아야 합니다.
...
>[!IMPORTANT]
>
>Google Sheets는 다음 예에서 값을 가져오는 데 사용되는 샘플 데이터베이스 시스템으로 교육용으로만 사용됩니다. Adobe는 프로덕션 환경에서 Google Sheets 사용을 지지하지 않습니다.
>
...
다음 에셋 수준 데이터 트리거 예는 ContextHub 구성 및 AEM Screens 채널에 대한 세그먼트 경로를 사용할 수 있는 Excel 시트와 같은 데이터 저장소를 설정하는 로컬 데이터베이스 이벤트를 보여줍니다.
...
1. **ContextHub로 이동**
   ...
   AEM 인스턴스로 이동하고 왼쪽 사이드바에서 도구 아이콘을 클릭합니다. 아래 그림과 같이 **사이트** —> **ContextHub**&#x200B;를 클릭합니다.
...
업그레이드 전 또는 후에 작성자의 다음 단계에 따라 게시해야 합니다.
   ...
   1. 제목 **을** Google ******** Google **Store 이름**&#x200B;으로 입력하고 **GoogleEsign 및 Google Store** 유형 **** ****&#x200B;으로 GoogleEtranthub StoreType asContexthub.genericjsonpV.JeckNext를 클릭합니다.
      ...
      >[!CAUTION]
      >AEM(Adobe Experience Manager) 6.4를 사용 중인 경우 **구성 제목** 을 **구문표** 로 **입력하고** 스토어 유형 **을** contexthub 일반-jsonp로 입력합니다.
...
1. **활동에서 세그먼트 만들기**
   ...
   1. 제목 **을** ValueCheck50 **** 및 **이름** 으로 **valuecheck50**&#x200B;으로입력합니다. 드롭다운에서 **타깃팅 엔진** 을 **AEM(ContextHub)** 으로 선택하고 **다음을 클릭합니다**.
...
Google **Sheet/value/1/0** 은 아래 그림에서 google 시트에 채워진 행 2와 열을 나타냅니다.
   ...
   1. 값을 **70** 으로 **입력합니다**.
      ...
      >[!NOTE]
      AEM에서는 세그먼트를 녹색으로 표시하여 Google Sheet에서 데이터의 유효성을 확인합니다.
...
AEM Screens 프로젝트에 대해 ContextHub를 구성한 후에는 다른 사용 사례를 따라 데이터가 트리거된 자산이 다양한 업계에서 중요한 역할을 하는 방식을 이해할 수 있습니다.
