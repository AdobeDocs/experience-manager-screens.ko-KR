...
>
>자세한 내용은 [Google 개인 정보 보호 백서를 참조하십시오.](https://www.google.com/chrome/privacy/whitepaper.html#speech)
...
1. 다음 예에서는 아래 그림과 같이 **VoiceDemo** 및 세 개의 시퀀스 채널 **Main**, **ColdDreks** 및 **HotDreks**&#x200B;이라는 데모 프로젝트를 쇼케이션합니다.
   ...
   아래 그림과 같이 세 개의 시퀀스 채널 **Main**, **ColdDrues** 및 **HotDreks**&#x200B;와 추가 1x2 분할 화면 채널 **SplitScreen**&#x200B;을 만들 수 있습니다.

   ![이미지](assets/voice-recognition/vr-emb-1.png)

1. 각 채널로 이동하고 컨텐츠를 추가합니다. 예를 들어 **VoiceDemo** —> **채널** —> **Main**&#x200B;으로 이동하여 채널을 선택합니다. 작업 표시줄에서 **편집**&#x200B;을 클릭하여 편집기를 열고 요구 사항에 따라 내용(이미지/비디오)을 추가합니다. 마찬가지로 **ColdDrinks** 및 **HotDrinks** 채널 모두에 컨텐츠를 추가합니다.
   ...
   **뜨거운 음료**:
   ...
   프로젝트에 화면 분할 채널을 추가한 경우 **SplitScreen**&#x200B;으로 이동하고 포함된 두 개의 시퀀스를 드래그하여 놓은 다음 아래 그림과 같이 **ColdDreks** 및 **HotDreks** 채널 모두에 경로를 추가합니다.
...
1. **기본 사항** 탭으로 이동하고 **태그** 필드에서 이미 기존 태그를 선택하거나 새 태그를 만듭니다.

   아래 그림에서와 같이 태그에 새 이름을 입력하여 새 태그를 만들고 `return` 키를 누르면 됩니다.

   ![이미지](assets/voice-recognition/vr-6.png)

   또는,

   또한 프로젝트에 대해 사전에 AEM 인스턴스에서 태그를 만들고 선택할 수도 있습니다. [태그 만들기](#creating-tags)에 설명된 단계에 따라 다음 그림과 같이 위치에서 태그를 선택하고 채널에 추가할 수 있습니다.

   ![이미지](assets/voice-recognition/vr-tag1.png)

1. 마찬가지로 **hot**&#x200B;이라는 이름의 태그를 **HotDrinks** 채널에 추가합니다.

1. 스크린 분할 채널을 사용하는 경우 아래 그림과 같이 태그(**hot** 및 **cold**)를 모두 **SplitScreen** 채널 속성에 추가합니다.
...
1. 채널 **Main**, **ColdDreks** 및 **HotDreks**&#x200B;을 **LobbyDisplay**&#x200B;에 할당합니다. 또한 프로젝트에 **SplitScreen** 채널을 사용하는 경우 해당 채널을 디스플레이에 지정해야 합니다.
...
1. 디스플레이에 채널을 지정했으면 **LobbyDisplay**&#x200B;로 이동하고 디스플레이를 선택합니다. 작업 표시줄에서 **속성**&#x200B;을 선택합니다.

1. **표시** 탭으로 이동하고 **컨텐트**&#x200B;에서 **음성 사용** 옵션을 활성화합니다.
...
**Main** 채널에서 해당 콘텐츠를 재생하고 있지만, *와 같은 키워드&#x200B;**hot**이 있는 단어를 사용할 때, 채널에서&#x200B;**HotDreks**채널의 내용을 재생하기 시작합니다.*

마찬가지로, *와 같은 키워드&#x200B;**cold**과 함께 단어를 사용하는 경우, 채널은&#x200B;**ColdDrues**채널의 내용을 재생하기 시작합니다.*

**분할 화면 채널에 대해 원하는 출력**

**Main** 채널에서 콘텐츠를 재생하고 있지만 키워드 **hot** 및 **cold**&#x200B;와 같은 키워드를 함께 사용하면 핫 및 cold 음료&#x200B;*의 메뉴를 볼 때 채널에서&#x200B;**SplitScreen**의 내용을 재생하기 시작합니다. channel.* *뒤로 기본 메뉴*&#x200B;를 지정하면 주 채널로 다시 전환됩니다.
