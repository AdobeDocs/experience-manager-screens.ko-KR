...
seo-description: AEM Screens용 간단한 "Hello World" 구성 요소를 만들기 위한 소개 자습서입니다. AEM Screens은 다른 AEM 제품의 많은 기존 디자인 패턴과 기술을 재사용합니다. 다음 자습서에서는 AEM Screens을 개발할 때 구체적인 차이점과 고려 사항을 강조 표시하기 위해 합니다.
...
자습서 단계 및 스크린샷은 **CRXDE-Lite**&#x200B;를 사용하여 수행됩니다. IDE를 사용하여 자습서를 완료할 수도 있습니다. IDE를 사용하여 [을 AEM과 함께 개발하는 방법에 대한 자세한 내용은 여기에서 확인할 수 있습니다.](https://helpx.adobe.com/experience-manager/kt/sites/using/getting-started-wknd-tutorial-develop/part1.html#eclipse-ide)


## 프로젝트 설정 {#project-setup}

스크린 프로젝트의 소스 코드는 일반적으로 다중 모듈 Maven 프로젝트로 관리됩니다. 자습서를 더 신속히 처리하기 위해 [AEM Project Archetype 13](https://github.com/Adobe-Marketing-Cloud/aem-project-archetype)을(를) 사용하여 프로젝트가 미리 생성되었습니다. Maven AEM Project Archetype을 사용하여 프로젝트를 만드는 방법에 대한 자세한 내용은 [여기에서 확인할 수 있습니다.](https://helpx.adobe.com/experience-manager/kt/sites/using/getting-started-wknd-tutorial-develop/part1.html#maven-multimodule)

1. [CRX 패키지 관리자](http://localhost:4502/crx/packmgr/index.jsp)를 사용하여 다음 패키지를 다운로드하여 설치합니다.
...
[파일 가져오기](assets/base-screens-weretail-runuiapps-001-snapshot.zip)
   ...
   **** 선택 사항(Eclipse 또는 다른 IDE를 사용하여 작업하는 경우)에는 아래 소스 패키지를 다운로드합니다. Maven 명령을 사용하여 프로젝트를 로컬 AEM 인스턴스에 배포합니다.
...
1. [CRX Package Manager](http://localhost:4502/crx/packmgr/index.jsp)에서 다음 두 패키지가 설치되었는지 확인합니다.
...
1. **screens-weretail-run.ui.apps** 패키지는 `/apps/weretail-run` 아래에 코드를 설치합니다.
   ...
   이 패키지에는 프로젝트의 사용자 지정 구성 요소를 렌더링하는 코드가 들어 있습니다. 이 패키지에는 구성 요소 코드와 필요한 JavaScript 또는 CSS가 들어 있습니다. 또한 이 패키지에는 프로젝트에 필요한 Java 코드가 들어 있는 **screens-weretail-run.core-0.0.1-SNAPSHOT.jar**&#x200B;도 포함되어 있습니다.
   ...
   **helloworld** 구성 요소는 현재 자리 표시자입니다. 자습서의 과정에는 작성자가 구성 요소에서 표시하는 메시지를 업데이트할 수 있는 기능이 추가됩니다.
...
1. **CRXDE-Lite** `http://localhost:4502/crx/de/index.jsp`(또는 선택한 IDE)에서 `/apps/weretail-run/components/content/helloworld.`로 이동합니다.
...
1. `/apps/weretail-run/components/content/helloworld` 아래에 `helloworld.html.` 파일을 만듭니다.
   ...
   */-->
   ...
   스크린 구성 요소는 사용 중인 [작성 모드](https://helpx.adobe.com/experience-manager/6-4/sites/authoring/using/author-environment-tools.html#PageModes)에 따라 두 개의 다른 렌더링이 필요합니다.

   1. **프로덕션**:미리 보기 또는 게시 모드 (wcmmode=disabled)
   1. **편집**:편집, 디자인, 스캐폴딩, 개발자 등 기타 모든 작성 모드에 사용됩니다.

   `helloworld.html`는 스위치 역할을 하며 현재 활성 상태인 작성 모드를 확인하고 다른 HTL 스크립트로 리디렉션합니다. 화면 구성 요소에서 사용하는 일반적인 규칙은 편집 모드에 대한 `edit.html` 스크립트 및 프로덕션 모드에 대한 `production.html` 스크립트를 포함하는 것입니다.

1. `/apps/weretail-run/components/content/helloworld` 아래에 `production.html.` 파일을 만듭니다.
   ...
   */-->
   ...
   위에 Hello World 구성 요소의 프로덕션 마크업이 있습니다. 구성 요소가 시퀀스 채널에서 사용되므로 `data-duration` 속성이 포함됩니다. `data-duration` 속성은 시퀀스 항목이 표시되는 시간을 알기 위해 시퀀스 채널에서 사용합니다.

   구성 요소는 텍스트가 있는 `div` 및 `h1` 태그를 렌더링합니다. `${properties.message}` 는 이름이 인 JCR 속성의 내용을 출력하는 HTL 스크립트의 일부입니다  `message`. 나중에 사용자가 `message` 속성 텍스트의 값을 입력할 수 있는 대화 상자가 만들어집니다.
...
1. `/apps/weretail-run/components/content/helloworld` 아래에 `edit.html.` 파일을 만듭니다.
   ...
   */-->
   ...
   대화 상자 메시지를 입력하지 않은 경우 두 번째 블록이 렌더링됩니다. `cq-placeholder` 및 `data-emptytext`은 이 경우 레이블 ***Hello World***&#x200B;를 자리 표시자로 렌더링합니다. 여러 로케일의 작성을 지원하기 위해 i18n을 사용하여 레이블의 문자열을 국제화할 수 있습니다.
...
1. **Hello World 구성 요소에 사용할 Screens 이미지 대화 상자 복사**
   ...
   1. `/apps/weretail-run/components/content/helloworld` 아래에 대화 상자를 붙여넣습니다.
...
1. **메시지의 탭을 포함하도록 Hello World 대화 상자를 업데이트합니다.**
   ...
   메시지의 텍스트 필드는 `message` 속성에 저장되며, Duration의 number 필드는 `duration` 속성에 저장됩니다. 이 두 속성은 모두 HTL이 `/apps/weretail-run/components/content/helloworld/production.html`에서 `${properties.message}` 및 `${properties.duration}`로 참조됩니다.
...
1. Hello World 구성 요소용 클라이언트 측 라이브러리에 대한 폴더를 만듭니다.
   ...
   `/apps/weretail-run/components/content/helloworld` 아래에 `clientlibs` 라는 새 폴더를 만듭니다.
...
1. `/apps/weretail-run/components/content/helloworld/clientlibs/shared/css/styles.less` 을 다음과 같이 채웁니다.
...
1. 프로덕션 clientlibrary의 `categories` 속성을 `cq.screens.components.production.`으로 업데이트하십시오.
   ...
   이렇게 하면 미리 보기/프로덕션 모드에 있는 경우에만 스타일이 로드됩니다.
...
1. `/apps/weretail-run/components/content/helloworld/clientlibs/production/css/styles.less` 을 다음과 같이 채웁니다.
...
세 번째 clientlibrary 카테고리:`cq.screens.components.edit` 을 사용하여 편집 전용 스타일을 구성 요소에 추가할 수 있습니다.
...
AEM Screens은 전역 변경 사항에 [정적 페이지 템플릿](https://helpx.adobe.com/experience-manager/6-5/sites/developing/using/page-templates-static.html) 및 [디자인 구성](https://helpx.adobe.com/experience-manager/6-4/sites/authoring/using/default-components-designmode.html)을 사용합니다. 디자인 구성은 채널에서 Parsys에 대해 허용되는 구성 요소를 구성하는 데 자주 사용됩니다. 가장 좋은 방법은 이러한 구성을 앱별 방식으로 저장하는 것입니다.
...
1. 디자인 폴더 아래에 `we-retail-run` 라는 새 노드를 만들고 `cq:Page` 유형을 사용합니다.
1. `we-retail-run` 페이지 아래에서 `nt:unstructured` 유형의 `jcr:content` 노드를 추가합니다. 다음 속성을 `jcr:content` 노드에 추가합니다.
...
1. AEM 시작 메뉴에서 **Screens** > **We.Retail Ru** n > 로 이동하고 **채널**&#x200B;을 선택합니다.
...
1. 템플릿 단계 - **시퀀스 채널**&#x200B;을 선택합니다.
   ...
   1. 속성 단계
   * 기본 탭 > 제목 = **유휴 채널**
   * 채널 탭 > 확인 **채널을 온라인 상태로 만들기**

   ![유휴 채널](assets/idle-channel.gif)

1. 유휴 채널에 대한 페이지 속성을 엽니다. 디자인 필드를 업데이트하여 이전 섹션에서 만든 디자인 페이지를 `/apps/settings/wcm/designs/we-retail-run,`로 가리킵니다.
...
1. 페이지 모드를 **편집**&#x200B;으로 전환합니다. 이제 Hello World 구성 요소를 페이지에 추가하고 다른 시퀀스 채널 구성 요소와 결합할 수 있습니다.
...
사용자 지정 구성 요소가 자산(이미지, 비디오, 글꼴, 아이콘 등), 특정 자산 표현물 또는 클라이언트측 라이브러리(css, js 등)와 같은 외부 리소스를 사용하는 경우, 이러한 구성 요소는 기본적으로 HTML 마크업을 번들하므로 오프라인 구성에 자동으로 추가되지 않습니다.

플레이어에 다운로드한 정확한 자산을 사용자 지정하고 최적화할 수 있도록, Adobe에서는 사용자 지정 구성 요소가 Screens의 오프라인 캐싱 로직에 종속성을 노출하는 확장 메커니즘을 제공합니다.

아래 섹션에서는 특정 프로젝트에 대한 `pom.xml`의 사용자 지정 오프라인 리소스 핸들러에 대한 템플릿과 최소 요구 사항을 소개합니다.
