...
seo-description: 다중 영역 레이아웃에서 사용자 지정 템플릿을 만드는 방법에 대해 알려면 이 페이지를 따르십시오.
...
* **Left20** 은 가로 및 세로 화면 크기의 20%를 덮는 왼쪽의 위쪽 영역을 나타냅니다.
* **** 가로는 화면 방향을 나타냅니다
* **** HD는 16:9의 종횡비를 나타냅니다
* **3** Zone은 디스플레이의 세 영역을 나타냅니다
...
1. **사용자 지정 템플릿**&#x200B;으로 명명된 AEM Screens 프로젝트를 만듭니다.

   ![이미지](/help/user-guide/assets/custom-multizone/custom-template2.png)

1. AEM 인스턴스에서 **CRXDE Lite** 도구 —> **CRXDE Lite**&#x200B;로 이동합니다.

1. **customtemplate**&#x200B;으로 명명된 **apps** 아래에 폴더를 만듭니다. 마찬가지로, 아래 그림과 같이 **customtemplate** 아래에 **template**&#x200B;이라는 이름의 다른 폴더를 만듭니다.

   ![이미지](/help/user-guide/assets/custom-multizone/custom-template1.png)

   >[!NOTE]
   >컨텐츠를 만들거나 편집하거나 노드에 복사할 때마다 CRXDE Lite의 작업 표시줄에서 **모두 저장**&#x200B;을 클릭하는 것이 좋습니다. 그렇지 않으면 업데이트를 커밋할 수 없습니다.

1. 왼쪽 막대 템플릿을 `/libs/screens/core/templates/splitscreenchannel/lbar-left`에서 `/apps/customtemplate/template`(으)로 복사합니다.

1. 복사한 **bar-left**(`/apps/customtemplate/template`)의 이름을 **my-custom-layout**로 변경합니다.
   ![이미지](/help/user-guide/assets/custom-multizone/custom-template3.png)

1. `/apps/customtemplate/template/my-custom-layout`로 이동하고 속성 **jcr:description**&#x200B;을 *Template for Left20-LandscapeHD3Zone* 및 **jcr:title**&#x200B;을 *Left20-LandscapeHD3Zone*&#x200B;으로 업데이트합니다.
...
1. 왼쪽 위 템플릿을 복사한 단계(4)를 참조하여 `my-custom-layout/jcr:content` 아래에 3개의 응답형 그리드를 표시합니다. *cq:cssClass* 속성의 각 응답형 그리드에 사용자 지정 css 클래스를 추가합니다. 예를 들어, *my-custom-layout—top-left* for *r1c1* 노드.
...
1. `/libs/settings/wcm/designs/screens`을 `/apps/settings/wcm/designs/`에 복사하고 복사된 디자인의 이름을 **custom-template-designs**&#x200B;로 변경합니다.

1. `/apps/settings/wcm/designs/custom-template-designs` 로 이동하고 **custom-template-designs**&#x200B;의 *jcr:title* 속성을 **customtemplate-design**&#x200B;으로 업데이트합니다.

1. `/apps/settings/wcm/designs/custom-template-designs` 로 이동하여 static.css 파일을 만듭니다.

1. 컨텐츠를 `static.css` 파일에 복사합니다.
...
1. `/apps/<project>/templates/my-custom-layout/jcr:content` 로 이동하고 *cq:designPath* 를 `/apps/settings/wcm/designs/customtemplate-designs` 로 업데이트하여 static.css에 구성된 스타일을 로드합니다
...
1. 사용자 지정된 템플릿으로 채널을 만든 후에는 편집기에서 채널에 자산을 추가할 수 있습니다. 다음 미리 보기는 사용자 지정 템플릿의 이미지를 보여줍니다.

   ![이미지](/help/user-guide/assets/custom-multizone/custom-template10.png)

## 이미지를 배경 레이어 {#inserting-image}로 삽입

이미지를 레이아웃에 배경 레이어로 삽입할 수 있습니다.

CSS 규칙을 조정하여 &quot;data-uri&quot;라고 하는 항목을 사용하고, CSS 파일에서 이미지(Base64 인코딩)를 직접 인라인으로 만들 수 있습니다. 이 경우 (13단계), *static.css*&#x200B;에서 만들 수 있습니다.
...
1. 이미지가 어떻게든 채널의 오프라인 구성에 포함되었는지 확인하십시오
1. &quot;data-uri&quot; 변형 대신 위의 CSS에서 이미지에 대한 직접 링크를 사용합니다
