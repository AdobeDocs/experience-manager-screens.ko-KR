...
>[!IMPORTANT]
>
>디스패처와 함께 둘 이상의 게시 인스턴스를 사용하려면 디스패처의 dispatcher.any 파일을 업데이트해야 합니다. 자세한 내용은 [고정 세션 활성화](dispatcher-configurations-aem-screens.md#enable-sticky-session)를 참조하십시오.

## 작성자 및 게시 인스턴스 구성 {#configuring-author-and-publish-instances}

>[!NOTE]
>
>작성자 및 게시 아키텍처 개요와 AEM 작성자 인스턴스에서 컨텐츠가 작성되고 앞으로 복제되는 방법에 대한 자세한 내용은 [작성자 및 게시 아키텍처 개요](author-publish-architecture-overview.md)를 참조하십시오.
...
1. **Replication** 폴더에서 작성자의 **에이전트**&#x200B;를 선택하고 **새로 만들기**&#x200B;를 클릭하여 새 표준 복제 에이전트를 만듭니다.
...
1. **편집**&#x200B;을 클릭하여 **에이전트 설정** 대화 상자를 열고 세부 정보를 입력합니다.
   ...
   >[!NOTE]
   >
   >복제 에이전트를 활성화하려면 **Enabled**&#x200B;을(를) 확인해야 합니다. 기본, 스크린 및 역 복제 에이전트에서 이 옵션을 선택해야 합니다.

   ![screen_shot_2019-02-25at30134pm](assets/screen_shot_2019-02-25at30134pm.png)

1. **전송** 탭으로 이동하고 **URI**, **사용자** 및 **암호**&#x200B;를 입력합니다.
...
1. pub2용 AEM Screens 복제 에이전트를 만듭니다. pub1용 Screens 복제 에이전트를 복사하고 이 포트를 pub2의 4504로 변경합니다.
...
게시 인스턴스의 경우 `https://:/system/console/topology`으로 이동합니다. You should see each publish instances represented in the topology unding **Outcome topology connectors** 아래에 표시됩니다.

#### 3단계:ActiveMQ 아르테미스 클러스터 {#step-setup-activemq-artemis-cluster} 설정
...
1. OSGi 콘솔에서 **MAIN** —> **Crypto Support**(`https://&lt;host&gt;:&lt;port&gt;/system/console/crypto`)으로 이동합니다.
...
#### ActiveMQ 아르테미스 클러스터 {#verify-activemq-artemis-cluster} 확인

각 게시 인스턴스의 아래 단계를 따르십시오.

1. OSGi 콘솔 -> 주 > ActiveMQ Artem `https://localhost:4505/system/console/mq`으로 이동합니다.
...
*/system/console/mq*&#x200B;에서 다음 구성이 표시되지 않으면 */system/console/mq*&#x200B;으로 이동하고 **다시 시작**&#x200B;을 클릭하여 브로커를 다시 시작합니다.
...
1. 별도의 브라우저 창을 실행합니다.
1. *웹 브라우저*&#x200B;을(를) 사용하여 Screens 플레이어로 이동하거나, 즉`https://localhost:4502/content/mobileapps/cq-screens-player/firmware.html`AEM Screens 앱을 실행합니다. 장치를 열면 장치의 상태가 등록되지 않음으로 표시됩니다.
...
1. **할당**&#x200B;을 클릭합니다.
1. 프로세스를 완료하려면 **마침**&#x200B;을 클릭하십시오. 이제 장치가 할당됩니다.
...
그 전에 아래 단계를 수행하여 장치 ID를 확인하십시오. 확인하려면 */home/users/screens/we-retail/devices* 경로를 사용하여 CRXDE Lite에서 장치 ID를 검색하십시오.
...
**발행물 관리** 기능을 사용하면 작성자가 장치에 게시하기 위한 컨텐츠 업데이트를 제공할 수 있습니다. 전체 AEM Screens 프로젝트 또는 채널, 위치, 디바이스, 애플리케이션 또는 일정 중 하나에 대해서만 컨텐츠를 게시/게시 취소할 수 있습니다. 이 기능에 대한 자세한 내용은 [온디맨드 콘텐츠 업데이트](on-demand-content.md)를 참조하십시오.
