...
작성자 및 두 개의 게시 인스턴스를 호스팅하는 간단한 예를 설정할 수 있습니다.
...
1. AEM 인스턴스 > hammer 아이콘 > 로 이동합니다. **작업** > **구성**.

   ![screen_shot_2019-02-25at24621pm](assets/screen_shot_2019-02-25at24621pm.png)

1. 다음을 클릭합니다. **복제** 왼쪽 탐색 트리에서.

   ![screen_shot_2019-02-25at24715pm](assets/screen_shot_2019-02-25at24715pm.png)

1. 다음을 클릭합니다. **작성자의 에이전트** 다음에서 **복제** 폴더 및 클릭 **신규** 새 표준 복제 에이전트를 만듭니다.

   ![screen_shot_2019-02-25at25400pm](assets/screen_shot_2019-02-25at25400pm.png)

1. 다음을 입력합니다. **제목** 및 **이름** 복제 에이전트를 생성할 수 있습니다. 그런 다음 **만들기**.
...
1. 다음에서 **에이전트 설정** 대화 상자에서 세부 정보를 입력합니다.

   >[!NOTE]
   >
   >사용자는 다음을 확인해야 합니다. **활성화됨** 복제 에이전트를 활성화합니다. 기본, 화면 및 역방향 복제 에이전트에서 이 옵션을 선택합니다.
...
1. pub1용 AEM Screens 복제 에이전트를 만듭니다. 기본적으로 포트 4503을 가리키는 Screens 복제 에이전트라는 이름의 에이전트가 있습니다. 활성화하십시오.
1. pub2용 AEM Screens 복제 에이전트를 만듭니다. pub1용 Screens 복제 에이전트를 복사하고 pub2용 4504를 가리키도록 포트를 변경합니다.

   >[!NOTE]
   >Screens 복제 에이전트를 구성하는 방법은 다음을 참조하십시오. [Screens 복제 에이전트 구성](https://experienceleague.adobe.com/en/docs/experience-manager-screens/user-guide/administering/configure-screens-replication).
...
1. 클릭 **Apache Sling Oak 기반 검색 서비스** 구성.
...
게시 인스턴스의 경우 다음으로 이동합니다. `https://:/system/console/topology`. 아래 토폴로지에 표시된 각 게시 인스턴스가 표시됩니다. **나가는 토폴로지 커넥터**.
...
1. 클릭 **Protect**.
1. 값 복사 **보호된 텍스트** 메모장 또는 텍스트 편집기에 연결합니다. 이 값은 ActiveMQ에 대한 OSGi 구성에서 사용할 수 있습니다.

각 게시 인스턴스에는 기본적으로 고유한 암호화 키가 있으므로 각 게시 인스턴스에서 이 단계를 수행하고 다음 구성을 위해 고유한 키를 저장합니다.
...
1. 클릭 **Apache ActiveMQ Artemis JMS 공급자** 구성
1. 다음 내용을 업데이트합니다.
...
1. OSGi 콘솔 > 메인 > ActiveMQ 아르테미스로 이동합니다. `https://localhost:4505/system/console/mq`.
...
1. 필드에 다음 변경 사항을 입력합니다.
   ...
   1. **텍스트**: 헬로 월드
   1. 보기 `error.log` 클러스터 전체에서 메시지가 전송되고 수신되었는지 확인할 수 있습니다.
...
1. 다음으로 이동 `https://localhost:4502/screens.html/content/screens/we-retail` 또는 프로젝트를 클릭하고 장치 > 장치 관리자로 이동합니다.
1. 클릭 **장치 등록**.
1. 클릭 **장치 등록**.
1. 등록할 장치를 클릭한 다음 **장치 등록**.
1. 등록 코드를 확인한 다음 **유효성 검사**.
1. 장치의 제목을 입력한 다음 를 클릭합니다. **등록**.

#### 3단계: 표시할 장치 할당 {#step-assigning-the-device-to-display}

1. 클릭 **디스플레이 할당** 이전 단계의 대화 상자에서 가져올 수 있습니다.
1. 에서 채널의 표시 경로를 클릭합니다. **위치** 폴더를 삭제합니다.
1. 클릭 **할당**.
...
1. 검색 **`screens-devices-master`** 그룹입니다.
...
1. 장치를 클릭하고 **활성화** 아래 그림과 같이 작업 표시줄에서 을 선택합니다.

![screen_shot_2019-02-21at111036am](assets/screen_shot_2019-02-21at111036am.png)

>[!NOTE]
>
>또는 장치를 활성화한 후 서버 URL을 편집하거나 업데이트할 수도 있습니다. 작업 표시줄에서 을(를) 클릭합니다 **서버 URL 편집**&#x200B;을 참조하십시오. 변경 사항이 AEM Screens 플레이어에 전파됩니다.
...
* *화면 장치 사용자* - 이 정보는 AEM 사용자로 저장되며 다음 위치에서 활성화할 수 있습니다. **도구** > **보안** > **사용자**. 사용자에게 &quot;screens&quot; 접두사에 긴 직렬화된 문자열이 붙습니다.

* *프로젝트* - AEM Screens 프로젝트
* *위치* - 장치가 연결된 위치입니다.
* *채널* - 위치에 표시 중인 하나 이상의 채널.
* *예약* - 일정을 사용하는 경우 이 일정이 게시되었는지 확인합니다.
...
1. 선택 **URL 편집** 작성자 인스턴스 URL에서 게시 인스턴스 URL 중 하나로 변경되었습니다.
...
1. Screens 플레이어에서 관리자 UI를 보고 왼쪽 상단 모서리를 길게 누르면 터치로 활성화된 AEM Screens 플레이어에서 또는 마우스를 사용하여 관리자 메뉴를 열 수 있습니다.
...
1. AEM Screens 프로젝트로 이동한 다음 **장치** 폴더를 삭제합니다.
1. 클릭 **장치 관리자** 작업 표시줄에서
1. 장치를 클릭한 다음 작업 표시줄에서 을 클릭합니다. **서버 URL 편집**&#x200B;을 참조하십시오. 변경 사항이 AEM Screens 플레이어에 전파됩니다.

![screen_shot_2019-02-07at31028pm](assets/screen_shot_2019-02-07at31028pm.png)

다음 **게시 관리** 기능을 사용하면 컨텐츠 업데이트를 작성자에서 게시로 장치에 전달할 수 있습니다. 전체 AEM Screens 프로젝트에 대한 콘텐츠 또는 채널, 위치, 장치, 애플리케이션 또는 일정 중 하나에 대한 콘텐츠만 게시/게시 취소할 수 있습니다. 이 기능에 대한 자세한 내용은 [온디맨드 콘텐츠 업데이트](on-demand-content.md).
...
**솔루션**
Bulk-offline-update-screens-service 사용자에 대한 읽기 권한 부여 및 `screens-devices-master` 모든 사용자를 위한 그룹 `/content/dam` 또는 더 제한적으로 사용하려는 경우 사용할 특정 에셋입니다.
