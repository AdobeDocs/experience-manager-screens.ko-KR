...
1. 다음 위치로 이동 [AEM Screens 플레이어 다운로드](https://download.macromedia.com/screens/) Tizen 플레이어를 다운로드할 페이지입니다.
...
Samsung 장치에서 아래 단계에 따라 장치에 AEM Screens 플레이어 설치를 완료합니다.
...
1. 이제 AEM Screens 플레이어가 Samsung 장치에 자동으로 설치되고 실행됩니다.
...
>호환되지 않는 브라우저 엔진이 있습니다. *SameSite=None* AEM 6.5에서 AEM 6.7로 발급한 로그인 토큰에 사용되는 속성입니다. 일반적으로 브라우저를 사용 가능한 최신 버전으로 업그레이드하여 문제를 해결할 수 있습니다. 스마트 디스플레이, 셋톱 박스 또는 내장 된 탐색 엔진이 있는 기타 장치와 같은 일부 경우에는 이러한 업그레이드가 불가능할 수 있습니다.

사용할 때 이러한 호환되지 않는 클라이언트를 면제하려면 아래 단계를 따르십시오. *SameSite=None*:

1. Adobe Experience Manager(AEM) 서비스 팩 6.5.7로 업그레이드하십시오.

1. AEM을 다시 시작한 후 다음으로 이동 `/system/console/configMgr` 및 검색 **Adobe Granite 토큰 인증 핸들러**. 에 대한 값 설정 **SameSite** 값: 까지 **없음**.

1. 새 옵션이 표시됩니다. *samesite 속성에서 제외될 사용자 에이전트*. 호환되지 않는 사용자 에이전트에 해당하는 정규 표현식으로 채웁니다. *SameSite=None* 특성.
   >[!NOTE]
   >다음을 참조하십시오 [SameSite=None: 알려진 호환되지 않는 클라이언트](https://www.chromium.org/updates/same-site/incompatible-clients) 을 참조하십시오. Tizen 플레이어의 경우 정규 표현식을 사용합니다. `(.*)Tizen(.*)`.

1. AEM 6.5.5 이상 인스턴스에 대해 Tizen 플레이어를 등록하면 콘텐츠를 정상적으로 등록하고 표시해야 합니다.

## Tizen 플레이어 원격 프로비저닝 {#remote-provisioning}

타이젠 플레이어를 원격으로 프로비저닝하면 큰 노력 없이 수백, 수천 개의 삼성 타이젠 디스플레이를 배포할 수 있습니다. 서버 URL 및 벌크 등록 코드 또는 기타 매개 변수를 사용하여 각 플레이어를 구성하는 번거로운 수작업을 피하고, Screens의 경우 클라우드 모드 및 클라우드 토큰을 as a Cloud Service으로 구성할 수 있습니다.
...
1. 필요한 경우 TLS를 설정합니다. 포트로 이동하고 서버에서 포트 번호를 선택한 다음 를 클릭합니다. **저장**.

1. 다음 위치로 이동 **장치** 을(를) 탭하고 방금 구성한 장치를 확인합니다. 장치가 검색되면 확인란을 클릭하고 을(를) 선택합니다 **승인**.
...
1. 장치가 승인되면 장치 목록에 표시됩니다. 을(를) 클릭합니다 *정보* 장치 상자에 있는 단추, 즉 **i**&#x200B;을 참조하십시오.

   >![이미지](/help/user-guide/assets/tizen/rms-6.png)

1. 디바이스 정보 대화 상자가 표시됩니다. 다음 항목 선택 **장치 정보** tab 키를 누른 다음 클릭 **편집**.

   >![이미지](/help/user-guide/assets/tizen/rms-5.png)

1. 장치 옵션을 편집하고 **설정** 탭. 다음으로 이동 **URL 런처** 섹션을 만들고 wgt 및 를 호스팅하는 URL을 입력합니다. `SSSP config file` 를 설치하려면 다음을 수행하십시오. `SSSP` 아래 그림과 같이 애플리케이션.

   ![이미지](/help/user-guide/assets/tizen/rms-9.png)

1. 클릭 **저장** 변경 사항이 표시 화면에 표시됩니다.
