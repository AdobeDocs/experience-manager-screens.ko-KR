...
Tizen 플레이어는 설치 및 실행 시(및 재부팅 때마다) `config.json` 파일의 설정을 확인하고 적용합니다.
...
>예를 들어 다시 부팅 일정에 대해 수동 구성을 허용하려면 정책 구성에 `rebootSchedule` 키를 지정하지 마십시오. 플레이어를 다시 로드할 때마다 정책 구성을 읽습니다.
