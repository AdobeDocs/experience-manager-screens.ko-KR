...
1. 다음 위치로 이동 [AEM Screens 플레이어 다운로드](https://download.macromedia.com/screens/) 타이젠 플레이어를 다운로드 할 수 있도록 페이지.
...
1. 아래로 스크롤하여 **다음 방법으로 재생** 옵션 및 변경 **URL 런처** 옵션을 선택합니다.
...
1. 새 옵션이 표시됩니다. *`User agents to be exempted from samesite attribute`*. 호환되지 않는 사용자 에이전트에 해당하는 정규 표현식으로 이 옵션 채우기 *SameSite=None* 특성.
...
1. 필요한 경우 TLS를 설정합니다. 포트로 이동하고 서버에서 포트 번호를 클릭한 다음 를 클릭합니다. **저장**.

1. 다음 위치로 이동 **장치** 를 탭하고 구성한 장치를 확인합니다. 장치를 찾으면 확인란을 클릭한 다음 을 클릭합니다 **승인**.
...
1. 장치가 승인되면 장치 목록에 표시됩니다. 클릭 *정보* 다음 그림과 같이 장치 상자에서 작업을 수행합니다.

   >![이미지](/help/user-guide/assets/tizen/rms-6.png)

1. 디바이스 정보 대화 상자가 표시됩니다. 다음을 클릭합니다. **장치 정보** tab 키를 누른 다음 클릭 **편집**.
