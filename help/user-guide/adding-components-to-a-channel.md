...
다음 예제에서는 응용 프로그램 구성 요소를 속성 미리 보기와 함께 포함하는 방법을 보여 줍니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><strong>원래</strong> 또는 <strong>단일</strong>으로 설정합니다. 값을 <strong>original</strong>으로 설정하면 하위 시퀀스가 상위 시퀀스의 각 주기에서 완전히 실행됩니다. 다른 가능한 값은 <strong>single</strong>이며, 이 값은 각 실행 시 하위 시퀀스의 한 항목만 표시합니다(예: 첫 번째 루프의 첫 번째 항목, 두 번째 루프의 2번째 항목 등).</td>
  </tr>
 </tbody>
</table>
...
>[!NOTE]
>
>이 구성 요소에서 추가하는 **클라이언트측 라이브러리** 및 **정적 파일**&#x200B;은 이미 구성된 **클라이언트측 라이브러리** 및 경험 조각의 **속성**&#x200B;에서 추가된 정적 파일에 추가됩니다.
...
| **시퀀스** |
| ***기간*** | 기본적으로 기간은 *8000ms*&#x200B;로 설정됩니다. 이미지의 재생 시간을 변경하려면 **지속 시간** 필드를 업데이트합니다. |
...
전환 아이콘을 선택하고 **구성**(렌치 아이콘)을 클릭하여 **전환** 대화 상자를 엽니다. 이 대화 상자에는 다음과 같은 3개의 탭이 있습니다.
