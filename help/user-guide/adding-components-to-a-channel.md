...
1. 채널을 선택합니다. 예를 들어, **`We.Retail In Store`** > **채널** > **유휴 채널**.

1. 선택 **편집** 작업 표시줄에서
1. AEM 편집기에서 **+** 아이콘을 클릭합니다.
...
AEM은 몇 가지 다른 구성 요소를 제공합니다. 다른 구성 요소(기본적으로 포함되지 않음)가 AEM Screens과 호환된다고 가정할 경우 언제든지 프로젝트에 추가할 수 있습니다.

다음 예는 AEM Screens 프로젝트에 Livefyre 구성 요소를 추가하는 방법을 보여줍니다.

1. 구성 요소를 추가할 채널을 선택합니다. 예를 들어, **`We.Retail In Store`** > **채널** > **유휴 채널**.

1. 선택 **편집** 작업 표시줄에서
1. 선택 **디자인** 모드.
1. 오른쪽의 전체 디자인 편집기를 선택하고 설정 기호를 선택하여 **Parsys 디자인** 대화 상자.
...
>[!NOTE]
>
>구성 요소의 등록 정보를 보려면 구성 요소를 선택하고 해머 아이콘을 선택하여 등록 정보를 열고 확인합니다.
...
| ***기간(밀리초)*** | 응용 프로그램 기간을 선택합니다. 기본적으로 duration은 -1로 설정되어 있으며, 이는 요소가 영원히(즉, 단일 페이지 애플리케이션) 실행됨을 의미합니다. 지속 시간 값을 0보다 높게 설정하면 지정된 지속 시간에 대한 요소가 표시된 후 다음 값으로 이동합니다. |
...
>[!NOTE]
>
>포함된 시퀀스에 대한 자세한 내용은 [포함된 시퀀스](embedded-sequences.md) 제작 화면 섹션에서 참조할 수 있습니다.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td>다음으로 설정 <strong>원본</strong> 또는 <strong>단일</strong>. 값을 로 설정 <strong>원본</strong> 는 하위 시퀀스가 상위 시퀀스의 각 사이클에서 완전히 실행됨을 의미합니다. 다른 가능한 값은 입니다. <strong>단일</strong>. 이러한 값은 각 실행 시 하위 시퀀스의 한 항목만 표시합니다. 예를 들어 첫 번째 루프의 첫 번째 항목과 두 번째 루프의 두 번째 항목이 있습니다.</td>
  </tr>
 </tbody>
</table>
...
동적 포함 시퀀스를 사용하면 채널 역할의 경우를 제외하고 위에서 언급한 것과 유사한 시퀀스를 추가할 수 있습니다.
...
경험 조각을 만들어 AEM Screens 프로젝트에 적용하는 방법에 대한 자세한 내용은 다음을 참조하십시오. [경험 조각 사용](experience-fragments-in-screens.md).
...
>[!NOTE]
>
>다음 **클라이언트 측 라이브러리** 및 **정적 파일** 이 구성 요소에서 추가하는 내용은 이미 구성된 항목 외에 **클라이언트 측 라이브러리** 및 경험 조각에서 추가된 정적 파일 **속성**.
...
>[!NOTE]
>
>기본적으로 이 시퀀스는 600밀리초로 설정됩니다. 다음을 사용하여 변환 시퀀스를 다른 값으로 업데이트할 수 있습니다. **시퀀스** 탭.
...
<table>
 <tbody>
  ...
  <tr>
   ...
   <td><p>비디오 종횡비가 화면에 맞지 않으면 렌더링 중 하나를 조정할 수 있습니다 <strong>contain</strong> 또는 <strong>커버</strong>.</p> <p><em>포함</em> 은 전체 비디오가 표시되고 누락된 영역이 검정색 테두리로 채워진다는 의미입니다.</p> <p><em>커버</em> 는 비디오가 뷰포트 전체를 포함하지만 측면에 오버플로되는 일부 부분이 숨겨져 있음을 의미합니다.</p> </td>
  </tr>
  <tr>
