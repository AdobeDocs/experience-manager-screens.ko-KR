...
단일 Dispatcher가 앞에 있는 여러 게시 인스턴스를 사용하려면 `dispatcher.any` 고정성을 활성화하는 파일입니다.
...
하나의 Dispatcher에 의해 앞에 하나의 게시 인스턴스가 있는 경우 로드 밸런서가 각 요청을 Dispatcher로 보낼 수 있으므로 Dispatcher에서 고착성을 활성화하는 것은 도움이 되지 않습니다. 이 경우 다음을 선택합니다 **사용** 위치: **끈적임** 아래 그림과 같이 로드 밸런서 수준에서 켤 필드입니다.

![이미지](/help/user-guide/assets/dispatcher/dispatcher-enable.png)

예를 들어 AWS ALB를 사용하는 경우 다음을 참조하십시오. [응용 프로그램 로드 밸런서의 대상 그룹](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-target-groups.html) ALB 수준에서 고착성을 활성화합니다. 하루 동안 고착성을 활성화합니다.
...
AEM Screens에서 타겟팅된 캠페인을 사용하는 경우 `segments.js file` AEM에 새 세그먼트를 추가하고 게시하므로 Dispatcher에서 제공하는 가 무효화되어야 합니다. 이 무효화 규칙이 없으면 새 타겟팅된 캠페인이 AEM Screens 플레이어에서 작동하지 않습니다(대신 기본 콘텐츠가 표시됨).
