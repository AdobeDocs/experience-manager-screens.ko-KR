...
description: 이 페이지를 따라 채널 만들기 및 관리에 대해 알아보십시오. 또한 채널 대시보드 및 채널의 콘텐츠 편집에 대해서도 설명합니다.
...
AEM Screens에 대한 프로젝트를 만든 후 아래 단계에 따라 프로젝트에 대한 새 채널을 만드십시오.
...
| 2x2 분할 화면 채널 | 4개 영역(행렬로 가로 및 세로로 분할)에서 자산을 볼 수 있습니다. |
...
>[!IMPORTANT]
>
> 콘텐츠를 만들고 채널에 추가하면 다음 단계로 위치를 만든 후 디스플레이를 만듭니다. 또한 해당 채널을 디스플레이에 할당해야 합니다. 자세한 내용은 섹션의 끝에 있는 아래 리소스를 참조하십시오.
...
1. 클릭 **편집** 작업 표시줄의 왼쪽 상단 모서리에서 시작하여 채널 속성을 편집합니다. 편집기가 열리고 게시할 채널에 에셋/구성 요소를 추가할 수 있습니다.
...
1. 선택 **비디오** 에셋에서 필요한 비디오를 드래그 앤 드롭합니다.
...
채널의 속성을 보거나 편집하려면 아래 단계를 따르십시오.

1. 편집할 채널을 클릭합니다.
1. 클릭 **속성** 작업 표시줄에서 채널 속성을 보거나 편집할 수 있습니다. 다음 탭에서는 옵션을 변경할 수 있습니다.

![속성](assets/properties.gif)

### 대시보드 보기 {#viewing-dashboard}

채널의 대시보드를 보려면 아래 단계를 따르십시오.

1. 편집할 채널을 선택합니다.
1. 클릭 **대시보드** 작업 표시줄에서 대시보드를 봅니다. 다음 **채널 정보**,**할당된 디스플레이**, 및 **보류 중인 시작** 아래 그림과 같이 패널이 열립니다.
...
(**...**&#x200B;의 ) **채널 정보** 작업 표시줄을 사용하여 속성을 보거나, 콘텐츠를 편집하거나, 채널의 캐시(오프라인 콘텐츠)를 업데이트할 수 있습니다.
...
채널 대시보드에서 이 옵션을 활성화하려면 다음 단계를 따르십시오.
...
1. 다음으로 채널 탐색 **테스트 프로젝트** —> **채널** —> **테스트 채널**.
   ...
   클릭 **대시보드** 를 클릭하여 플레이어의 상태를 확인합니다. 다음 **채널 정보** 패널은 채널이 온라인인지 오프라인인지에 대한 정보를 제공합니다.
...
>[!NOTE]
>채널을 다시 오프라인으로 구성하려면 다음에서 개발자 모드 옵션의 선택을 해제합니다. **속성** 탭(단계 (3)에 표시됨) 다음에 나오는 **채널 정보** 패널 클릭 **오프라인 컨텐츠 업데이트**&#x200B;을 참조하십시오.
...
할당된 디스플레이 패널에는 채널과 연결된 디스플레이가 표시됩니다. 이 레이아웃은 해상도와 함께 지정된 디스플레이의 스냅샷을 제공합니다.

연결된 디스플레이가에 나열됩니다. **할당된 디스플레이** 패널, 아래 표시:
