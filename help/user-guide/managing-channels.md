...
| 2X1 분할 화면 채널 | 두 개의 영역(세로로 분할)에서 자산을 볼 수 있습니다. |
...
1. 개발자 **모드** (채널이 온라인 상태가 되도록 강제) **를** 확인하여 채널을 온라인 상태로 만듭니다.
...
>[!NOTE]
>
>채널을 다시 오프라인으로 구성하려면 속성 탭( **3단계 참조)에서** 개발자 모드 옵션을 선택 취소한 다음 **채널 정보** 패널에서 **아래 그림과 같이**&#x200B;오프라인콘텐츠업데이트를클릭합니다.
