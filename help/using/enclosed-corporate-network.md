...
*충분한 대역폭은* 연결된 AEM 화면의 양과 네트워크 내의 다른 소비자(예: 스마트폰, 태블릿, 계산원, 컴퓨터 또는 게스트 Wi-Fi 네트워크) 사용에 따라 달라집니다.
...
Wi-Fi 솔루션이 화면을 인터넷 링크에 연결할 것으로 예상되는 경우 최신 Wi-Fi 표준을 최소 `IEEE 802.11g` 로 사용하는 것이 좋습니다. 이 표준은 최대 54Mbps의 연결을 지원합니다. 이와 같은 *새로운* 표준은 `802.11h-n` 품질이 더 좋습니다. Wi-Fi 반복이 필요한 경우 Google Nest Mesh Wi-Fi 등과 같은 Mesh Wi-Fi 액세스 포인트 기술을 강력히 권장합니다.
다른 Wi-Fi 반복 기술은 전체 네트워크에서 대역폭이 크게 손실됩니다.
